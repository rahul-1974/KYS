...
// ---- Haut.AI ----
let hautToken = null, hautTokenExp = 0;
...
async function analyzeSkin(imageBase64) {
  if (!C.HAUT_EMAIL || !imageBase64) return demoSkin();
  ...
  const sub = await (await fetch(`${base}/subjects/`, { method:'POST', headers:hh, body:JSON.stringify({ name:`kys_${Date.now()}` }) })).json();
  const batch = await (await fetch(`${base}/subjects/${sub.id}/batches/`, { method:'POST', headers:hh })).json();
  await (await fetch(`${base}/subjects/${sub.id}/batches/${batch.id}/images/`, {
    method:'POST', headers:hh,
    body: JSON.stringify({ image: imageBase64, image_name: `selfie_${Date.now()}.jpg` }),
  })).json();
  ...
  for (let i = 0; i < 12; i++) {
    ...
    const img = await (await fetch(`${base}/subjects/${sub.id}/batches/${batch.id}/images/`, { headers:hh })).json();
    if (img && img.length > 0) {
      const imgId = img[0].id;
      const rr = await (await fetch(`${base}/subjects/${sub.id}/batches/${batch.id}/images/${imgId}/results/`, { headers:hh })).json();
      if (rr && Array.isArray(rr) && rr.length > 0) { results = rr; break; }
    }
  }
  if (!results) return { ...demoSkin(), source: 'timeout' };
  return parseHaut(results);
}

function parseHaut(results) {
  const m = {};
  for (const r of results) {
    if (!r.result) continue;
    if (r.result.algorithms) { for (const a of r.result.algorithms) m[a.tech_name||'']=a; }
    else m[r.application_tech_name||r.algorithm_tech_name||'']=r.result;
  }
  const skinType = xCat(m,'skin_type')||'Combination';
  const concerns = [
    {name:'Acne',score:xS(m,'acne'),tech:'acne'},
    {name:'Pigmentation',score:xS(m,'pigmentation'),tech:'pigmentation'},
    {name:'Open pores',score:xS(m,'pores'),tech:'pores'},
    {name:'Redness',score:xS(m,'redness'),tech:'redness'},
    {name:'Fine lines',score:xS(m,'lines'),tech:'lines'},
    {name:'Dark circles',score:xS(m,'eye_area_condition')||xS(m,'dark_circles'),tech:'dark_circles'},
    {name:'Uneven texture',score:100-(xS(m,'uniformness')||70),tech:'uniformness'},
  ].filter(c=>c.score>15).sort((a,b)=>b.score-a.score).slice(0,4)
   .map(c=>({...c,severity:c.score>65?'Severe':c.score>40?'Moderate':'Mild'}));
  const oilMap={Oily:78,Combination:62,Normal:45,Dry:22};
  return { source:'haut_ai', skinType, skinTone:xCat(m,'skin_tone')||'Medium',
    perceivedAge:Math.round(xV(m,'perceived_age')||28), hydration:Math.round(xV(m,'hydration')||45),
    oiliness:oilMap[skinType]||50, concerns };
}
function xCat(m,k){for(const[n,v]of Object.entries(m)){if(n.includes(k)){if(v?.main_metric?.value)return v.main_metric.value;if(v?.result?.main_metric?.value)return v.result.main_metric.value}}return null}
function xV(m,k){for(const[n,v]of Object.entries(m)){if(n.includes(k)){if(v?.main_metric?.value!==undefined)return parseFloat(v.main_metric.value);if(v?.result?.main_metric?.value!==undefined)return parseFloat(v.result.main_metric.value)}}return null}
function xS(m,k){for(const[n,v]of Object.entries(m)){if(n.includes(k)){if(v?.main_metric?.value!==undefined)return parseFloat(v.main_metric.value)||0;if(v?.sub_metrics)for(const s of v.sub_metrics)if(s.tech_name?.includes('score')||s.tech_name?.includes('severity'))return parseFloat(s.value)||0}}return 0}
...
async function getWeather(lat, lon) {
  ...
  return { temperature:Math.round(v.temperature??34), humidity:Math.round(v.humidity??45), uvIndex:Math.round(v.uvIndex??8), dewPoint:Math.round(v.dewPoint??15) };
}
...
async function getPollution(lat, lon) {
  ...
  return { aqi:p.aqius||180, pm25:p.pm25||90, mainPollutant:p.mainus||'p2', city:d.data?.city||'Unknown' };
}
...
function buildForecast(lat, lon) {
  ...
  const forecast=[];
  for(let i=1;i<=3;i++){
    const mi=(month+i)%12, mName=months[mi];
    let temp,humidity,uv,aqi,risk;
    if(isDelhiZone){
      const dd=[{t:15,h:55,u:4,a:250,r:'Extreme pollution + cold dryness'},{t:20,h:45,u:5,a:200,r:'Pollution easing + dry skin risk'},{t:28,h:35,u:8,a:165,r:'Rising UV + moderate pollution'},{t:36,h:25,u:10,a:150,r:'UV damage + dehydration'},{t:42,h:20,u:11,a:140,r:'Extreme UV + severe dryness'},{t:36,h:65,u:8,a:110,r:'Monsoon humidity + fungal risk'},{t:33,h:80,u:6,a:95,r:'High humidity + excess sebum'},{t:32,h:82,u:6,a:90,r:'Peak humidity + breakout risk'},{t:32,h:65,u:7,a:120,r:'Post-monsoon + pollution rising'},{t:30,h:50,u:6,a:280,r:'Diwali pollution spike + dryness'},{t:22,h:45,u:4,a:350,r:'Severe pollution + barrier damage'},{t:16,h:55,u:3,a:300,r:'Winter smog + water loss'}][mi];
      temp=dd.t;humidity=dd.h;uv=dd.u;aqi=dd.a;risk=dd.r;
    } else if(isMumbaiZone){
      temp=[27,28,30,32,33,31,29,29,30,32,31,29][mi]; humidity=[60,58,62,68,72,82,88,87,82,72,64,60][mi]; uv=[7,8,9,10,10,8,6,6,8,8,7,6][mi]; aqi=[90,85,95,100,95,70,55,50,65,110,120,100][mi]; risk=['Moderate UV + coastal humidity','Rising UV + salt air','High UV + dust','Extreme UV + heat','UV peak + dehydration','Monsoon + fungal risk','Peak humidity + breakouts','Humidity + bacterial risk','Post-monsoon transition','Pollution rise','Moderate pollution','Mild winter'][mi];
    } else {
      temp=[25,27,30,33,35,30,28,28,29,29,27,25][mi]; humidity=[50,45,40,35,40,65,75,78,70,60,50,50][mi]; uv=[6,7,9,10,10,8,6,6,7,7,6,5][mi]; aqi=[100,95,110,120,115,80,60,55,75,120,140,120][mi]; risk='Seasonal change — check local conditions';
    }
...
function recommend(skinType,concerns=[],hydration=50,oiliness=50,weather={},pollution={}){
  ...
  const hiPol=pollution?.aqi>120, hiUV=weather?.uvIndex>=7;
  ...
  for(const cat of['cleanser','serum','spf','moisturizer','night']){
    ...
    if(best&&best.score>20){
      let reason='';
      for(const c of topTechs){if(best.concerns.includes(c)){reason=`Targets ${concerns.find(x=>(x.tech||x.name.toLowerCase())===c)?.name||c}`;break;}}
  ...
  if(bonus&&bonus.score>30)routine.push({name:bonus.name,price:`₹${bonus.price.toLocaleString('en-IN')}`,reason:bonus.desc,img:bonus.img,match:bonus.matchPct,category:bonus.cat});
  return routine.sort((a,b)=>b.match-a.match);
}

// ---- Advisory ----
function buildAdvisory(skinType,concerns,weather,pollution,forecast){
...
// ---- Demo data ----
function demoSkin(){
  return{source:'demo',skinType:'Combination',skinTone:'Medium',perceivedAge:28,hydration:42,oiliness:68,
    concerns:[{name:'Pigmentation',score:62,severity:'Moderate',tech:'pigmentation'},{name:'Open pores',score:48,severity:'Moderate',tech:'pores'},{name:'Uneven texture',score:38,severity:'Mild',tech:'uniformness'},{name:'Dark circles',score:35,severity:'Mild',tech:'dark_circles'}]};
}
function demoEnv(){
  return{weather:{temperature:34,humidity:45,uvIndex:9,dewPoint:15},pollution:{aqi:186,pm25:92,mainPollutant:'p2',city:'New Delhi'},
    forecast:[{month:'Apr',temp:'38°C',humidity:'25%',uv:10,aqi:165,risk:'UV damage + dehydration'},{month:'May',temp:'42°C',humidity:'20%',uv:11,aqi:148,risk:'Extreme UV + dryness'},{month:'Jun',temp:'36°C',humidity:'65%',uv:8,aqi:112,risk:'Humidity surge + fungal risk'}],
    skinImpact:[{factor:'UV Radiation',level:'High',effect:'Accelerates pigmentation. SPF 50+ essential.'},{factor:'Air Pollution',level:'Severe',effect:'PM2.5 causes oxidative stress.'},{factor:'Low Humidity',level:'High',effect:'Accelerates trans-epidermal water loss.'}]};
}

module.exports = { analyzeSkin, getWeather, getPollution, buildForecast, skinImpact, recommend, buildAdvisory, demoSkin, demoEnv };
